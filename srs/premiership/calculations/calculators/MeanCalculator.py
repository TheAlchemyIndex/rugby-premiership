...
from srs.premiership.calculations.helpers.ColumnHelper import two_columns_mean, three_columns_mean, \
    mean_previous_5_scores, mean_previous_3_scores_opposition
...
def calc_conditional_means_three_cols(df):
    ...
    avg_home_score_by_team_against_opponent = three_columns_mean(df, OriginalColumns.TEAM2_NAME,
                                                                 OriginalColumns.TEAM1_NAME,
                                                                 OriginalColumns.TEAM1_SCORE,
                                                                 CalculatedColumns.AVG_HOME_SCORE_BY_TEAM_AGAINST_OPPONENT)
    ...
    avg_away_score_by_team_against_opponent = three_columns_mean(df, OriginalColumns.TEAM1_NAME,
                                                                 OriginalColumns.TEAM2_NAME,
                                                                 OriginalColumns.TEAM2_SCORE,
                                                                 CalculatedColumns.AVG_AWAY_SCORE_BY_TEAM_AGAINST_OPPONENT)
    ...
    new_df = pd.merge(new_df, avg_home_score_by_team_by_venue, on=[OriginalColumns.TEAM1_NAME, OriginalColumns.VENUE],
                      how="left")
    new_df = pd.merge(new_df, avg_home_score_by_team_against_opponent,
                      on=[OriginalColumns.TEAM1_NAME, OriginalColumns.TEAM2_NAME],
                      how="left")
    ...
    new_df = pd.merge(new_df, avg_away_score_by_team_by_venue, on=[OriginalColumns.TEAM2_NAME, OriginalColumns.VENUE],
                      how="left")
    new_df = pd.merge(new_df, avg_away_score_by_team_against_opponent,
                      on=[OriginalColumns.TEAM2_NAME, OriginalColumns.TEAM1_NAME],
                      how="left")
    new_df.fillna('N/A', inplace=True)
    new_df = new_df.round(2)
    return new_df
